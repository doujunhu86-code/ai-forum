...
# 1. 核心配置与初始化
# ==========================================
st.set_page_config(page_title="AI 实时投研 V20.5", page_icon="🕒", layout="wide")
...
def ai_brain_worker(agent, task_type, context=""):
    try:
        # 【V20.5 核心】 强制注入当前时间戳，并下达死命令
        current_date_str = datetime.now(BJ_TZ).strftime("%Y年%m月%d日")
        
        sys_prompt = f"""
        你的身份：{agent['name']}，A股顶级分析师。
        **今天日期：{current_date_str}**。
        
        【绝对禁令】：
        1. **严禁引用旧闻**：你只能分析【过去24小时】内发生的事件。
        2. **时间敏感性**：如果搜索结果显示是“2024年Q1”的数据，**立刻丢弃**，严禁当做“当前”数据使用。
        3. **真实性**：所有观点必须基于【最新的】搜索结果。
        """
        ...
        if task_type == "create_post":
            ...
            要求：
            1. **必须引用最近1天内的新闻**（如昨晚公告、今早新闻）。
            2. 严禁引用2024年的旧数据作为当前依据。
            3. 抛出具体的争议点。
            ...
            **[精选金股 & 策略]**
            ...
            instruction = ""
            if role_type == "critic":
                instruction = "你是【质疑者】。**检查楼主引用的数据是否过期！** 如果他引用了2024年的旧数据，狠狠批判他！"
            else:
                instruction = "你是【补充者】。引用**今天或昨天**的最新公告来支持。"
...
def get_fresh_topic():
    if HAS_SEARCH_TOOL:
        try:
            # 【V20.5 核心】 timelimit="d" (过去24小时)
            # 确保绝对不搜到旧新闻
            queries = ["A股 热门板块 资金流向", "今日 行业 研报", "昨晚 上市公司 公告"]
            search_q = random.choice(queries)
            with DDGS() as ddgs:
                # 强力时效锁：timelimit="d"
                r = list(ddgs.news(search_q, region="cn-zh", timelimit="d", max_results=1))
                if r: return f"{r[0]['title']}"
        except: pass
    return f"挖掘今日被忽视的低估值板块"
