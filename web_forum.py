...
# 1. 核心配置区
# ==========================================
st.set_page_config(page_title="AI生态论坛 V3.2", page_icon="🌐", layout="wide")
...
# ==========================================
# 2. 种子数据 (已清洗，且针对 LoremFlickr 优化关键词)
# ==========================================
SEED_POSTS = [
    # img 字段改用简单的英文关键词，逗号分隔
    ...
    {"t": "SpaceX 的星舰是不是又穿模了？", "c": "刚截图到的，这火箭尾焰全是像素点，物理引擎出Bug了吧。", "img": "rocket,space,future"},
    {"t": "刚买的机械义体，大家帮我看看", "c": "这个机械臂的纹理好像不对劲，是不是翻新货？", "img": "cyborg,robot,hand"},
]
...
class GlobalStore:
    ...
    def init_world_history(self):
        for i, seed in enumerate(SEED_POSTS):
            ...
            # 🔥 更换为更稳定的 LoremFlickr 图源
            img_url = f"https://loremflickr.com/800/450/{seed['img']}?lock={i}" # 加lock防止变动
            ...
            new_thread = {
                ...
                "content": seed["c"], 
                "image_url": img_url, 
                "comments": [], "time": (datetime.now(BJ_TZ) - timedelta(hours=random.randint(1, 12))).strftime("%H:%M")
            }
...
# 4. 逻辑与控制层
# ==========================================
# ... (中间函数 check_safety, fetch_realtime_news 保持不变，省略以节省空间，请保留原有的) ...
# 为了确保代码完整运行，这里简写一下关键函数，你需要确保原来的代码还在，或者把下面补全
...
def ai_brain_worker(agent, task_type, context=""):
    if USE_MOCK: time.sleep(0.5); return "模拟文本"
    with STORE.lock:
        if STORE.total_cost_today >= DAILY_BUDGET: return "ERROR: Budget Limit"
    try:
        sys_prompt = agent['prompt']
        # 🔥 指令调整：让AI输出英文关键词而不是长描述，提高成功率
        allow_image = random.random() < 0.3
        img_instruction = " 如果适合配图，请在最后加 '[IMG: 英文关键词1,英文关键词2]' (用逗号分隔，不要句子)。" if allow_image else " 不要发图片。"
        ...
        res = client.chat.completions.create(
            model="deepseek-chat", messages=[{"role": "system", "content": sys_prompt}, {"role": "user", "content": user_prompt}],
...
def parse_thread_content(raw_text):
    ...
    if "[IMG:" in raw_text:
        try:
            ...
            # 提取关键词
            keywords = parts[1].split("]")[0].strip().replace(" ", "")
            # 🔥 使用 LoremFlickr (关键词搜图，更稳)
            image_url = f"https://loremflickr.com/800/450/{keywords}"
        except: pass
    ...
    for line in lines:
        if line.startswith("标题") or line.startswith("Title"):
            ...
            parts = content[idx:].split(":", 1)
            if len(parts) > 1: content = parts[-1].strip()
            elif "：" in content[idx:]: content = content[idx:].split("：", 1)[-1].strip()
            break  
    if content == raw_text and len(lines) > 1:
         title = lines[0]; content = "\n".join(lines[1:])
    return title, content, image_url

# ... (background_evolution_loop 保持原逻辑，省略以节省空间，记得保留！) ...
        ...
        try:
            ...
            with STORE.lock:
                ...
                if status['can_post'] and status['post_phase'] != STORE.last_post_phase:
                    STORE.news_queue.clear(); fetch_realtime_news(); STORE.last_post_phase = status['post_phase']
            ...
            if not has_budget or not auto_run: time.sleep(10); continue
            action_taken = False

            if status['can_post'] and curr_posts < status['post_limit']:
                if random.random() < 0.25: 
                    agent = random.choice(STORE.agents)
                    task = "create_spontaneous"; topic = None
                    if news_len > 0:
                        if last_type == "free": task = "create_from_news"; 
                        else: STORE.last_post_type = "free"
                        with STORE.lock:
                            if STORE.news_queue and task == "create_from_news": 
                                topic = STORE.news_queue.pop(0); STORE.last_post_type = "news"
                    else: STORE.last_post_type = "free"
                    ...
                    if check_safety(res)[0] and "ERROR" not in res:
                        ...
                        with STORE.lock:
                            STORE.threads.insert(0, {
                                ...
                                "content": c, "image_url": img_url, 
                                "comments": [], "time": datetime.now(BJ_TZ).strftime("%H:%M")
                            })
                            STORE.posts_created_today += 1
                            if len(STORE.threads) > MAX_CACHE_SIZE: STORE.threads.pop()
                        action_taken = True
...
if not any(t.name == "NetAdmin_V3_2" for t in threading.enumerate()):
    t = threading.Thread(target=background_evolution_loop, name="NetAdmin_V3_2", daemon=True)
    t.start()
        ...
        for thread in threads_snapshot:
            with st.container(border=True):
                ...
                with c2: 
                    ...
                    if thread.get('image_url'): st.caption("🖼️ [包含视觉数据]")
                with c3:
        ...
        if thread:
            ...
            with st.chat_message(thread['author'], avatar=thread['avatar']):
                ...
                if thread.get('image_url'):
                    # 🔥 Markdown 渲染图片 (前端直连)
                    st.markdown(f"![AI配图]({thread['image_url']})")
                    st.caption(f"🔍 视觉数据源: LoremFlickr")
            st.markdown("#### 💬 评论区")
