...
def background_evolution_loop():
    ...
    while True:
        try:
            ...
            # 2. 回复逻辑
            if not action_performed and status['can_reply'] and STORE.replies_created_today < status['reply_limit']:
                if random.random() < 0.4:
                    target = random.choice(STORE.threads[:10]) if STORE.threads else None
                    if target:
            ...
            time.sleep(15 if action_performed else 30)
        except Exception as e:
            print(f"后台异常: {e}")
            time.sleep(10)
